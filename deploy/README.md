...
# 2. 给脚本执行权限
chmod +x scripts/install.sh scripts/start-api.sh
...
| 步骤 | 说明 |
|------|------|
| 创建目录结构 | `/opt/gov-platform/{api,web,uploads,logs}` |
| 复制 API 文件 | 后端代码和配置 |
...
# 2. 编辑 API 配置（设置数据库密码等）
vi /opt/gov-platform/api/.env

# 3. 复制构建好的前端文件到 web 目录
cp -r dist/* /opt/gov-platform/web/

# 4. 配置并启动 Nginx
cp nginx/gov-platform.conf /etc/nginx/conf.d/
