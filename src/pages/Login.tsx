  ...
  return (
    <div className="min-h-screen bg-secondary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <Card className="w-full max-w-md">
        <CardHeader style={{ textAlign: 'center' }}>
          <div style={{ width: '4rem', height: '4rem', borderRadius: '50%', margin: '0 auto 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="bg-gov-red-light">
            <span className="text-primary" style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>政</span>
          </div>
          <CardTitle className="text-2xl" style={{ textAlign: 'center' }}>党政办公平台</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {isOfflineMode() && (
            <div style={{ marginBottom: '1rem' }}>
              <Button
                ...
                variant="default"
                className="w-full"
                style={{ height: '3rem', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                disabled={ssoLoading}
              ...
              >
                {ssoLoading ? (
                  ...
                  <span style={{ display: 'inline-flex', alignItems: 'center' }}>
                    <Shield className="w-5 h-5" style={{ marginRight: '0.5rem' }} />
                    信任体系单点登录
                  </span>
                )}
              </Button>
              <div style={{ position: 'relative', padding: '0.5rem 0', marginTop: '1rem' }}>
                <Separator />
                <span style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', paddingLeft: '0.75rem', paddingRight: '0.75rem' }} className="bg-card text-xs text-muted-foreground">
                  或使用账号密码
                </span>
              </div>
            </div>
          ...
          <form onSubmit={handleLogin}>
            <div style={{ marginBottom: '1rem' }}>
              <Label htmlFor="mobile" style={{ display: 'block', marginBottom: '0.5rem' }}>账号</Label>
              <Input
            ...
            <div style={{ marginBottom: '1rem' }}>
              <Label htmlFor="password" style={{ display: 'block', marginBottom: '0.5rem' }}>密码</Label>
              <Input
                id="password" type="password" placeholder="请输入密码"
                value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading}
              />
            </div>
            <Button type="submit" variant={isOfflineMode() ? "outline" : "default"} className="w-full" disabled={loading} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {loading ? "登录中..." : (
                <span style={{ display: 'inline-flex', alignItems: 'center' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: '1rem' }} className="text-sm text-muted-foreground">默认密码：123456</p>
        </CardContent>
      </Card>
    </div>
  );
