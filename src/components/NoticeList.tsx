...
const NoticeList = () => {
  ...
  const fetchImages = async () => {
    const {
      data,
      error
    } = await supabase.from("notice_images").select("id, image_url, title").eq("is_active", true).order("sort_order", {
      ascending: true
    });
    if (!error && data) {
      setImages(data);
  ...
  return <div className="gov-card h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 py-0">
        ...
        >
            ...
            {/* 轮播控制按钮 */}
            {images.length > 1 && <>
                <Button variant="ghost" size="icon" className="absolute left-1 top-1/2 -translate-y-1/2 z-20 bg-black/30 hover:bg-black/50 text-white rounded-full w-6 h-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={goToPrevImage}>
                  <ChevronLeft className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 z-20 bg-black/30 hover:bg-black/50 text-white rounded-full w-6 h-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={goToNextImage}>
                  <ChevronRight className="w-4 h-4" />
                </Button>

                {/* 底部指示器 */}
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 flex gap-1">
                  {images.map((_, index) => <button key={index} className={`w-1.5 h-1.5 rounded-full transition-all ${index === currentImageIndex ? "bg-white" : "bg-white/50 hover:bg-white/70"}`} onClick={() => setCurrentImageIndex(index)} />)}
                </div>
              </>}
          </div>}
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded ${getDisplaySecurityLevel(selectedNotice?.security_level) === '机密' ? 'bg-red-100 text-red-700' : getDisplaySecurityLevel(selectedNotice?.security_level) === '秘密' ? 'bg-orange-100 text-orange-700' : getDisplaySecurityLevel(selectedNotice?.security_level) === '内部' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
