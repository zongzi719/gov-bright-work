...
const CompletedList = () => {
  ...
  // Fetch completed items for current user
  const fetchCompletedItems = async () => {
    ...
    const { data, error } = await supabase
      ...
      .neq("process_result", "cc_notified") // 排除抄送的待办（抄送单独显示）
    ...
    if (error) {
      ...
      setCompletedItems(data as unknown as TodoItem[]);
      ...
      <div className="gov-card h-full flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2">
            <h2 className="gov-card-title">已办理</h2>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <TodoDetailDialog
        ...
        onApprovalComplete={() => fetchCompletedItems()}
      />
    </>
  );
};
