...
const WorkPanel = () => {
  ...
  const renderPendingItem = (item: TodoItem) => {
    ...
    const { department } = getDisplayInfo(item);
    ...
    return (
      ...
      >
        ...
        <div className="ml-3 text-xs text-muted-foreground truncate mt-0.5">
          {department}
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <span className={`text-[10px] px-1 py-0.5 rounded ${color}`}>{label}</span>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between px-2 py-1.5 border-b border-border flex-shrink-0">
            <TabsList className="bg-transparent gap-0 p-0 h-auto">
              <TabsTrigger
                value="pending"
                className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary px-2 py-1 text-sm rounded-sm"
              >
                待办({pendingCount})
              </TabsTrigger>
              <TabsTrigger
                ...
                className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary px-2 py-1 text-sm rounded-sm"
              >
                已办({completedItems.length})
              </TabsTrigger>
              <TabsTrigger
                value="cc"
                className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary px-2 py-1 text-sm rounded-sm"
              >
                抄送({ccUnreadCount > 0 ? ccUnreadCount : ccItems.length})
              </TabsTrigger>
            </TabsList>
