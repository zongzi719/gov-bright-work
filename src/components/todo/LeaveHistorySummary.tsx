  ...
  return (
    <div className="mt-4 p-3 bg-amber-50 rounded-md border border-amber-200 text-sm text-foreground leading-relaxed space-y-2">
      ...
      <p className="text-muted-foreground">
        剩余可用：事假 <strong>{fmt(personalRemain)}</strong> 天；病假 <strong>{fmt(sickRemain)}</strong> 天；年假 <strong>{fmt(annualRemain)}</strong> 天；探亲假 <strong>{fmt(familyVisitRemain)}</strong> 天；婚丧假 <strong>{fmt(marriageBereavementRemain)}</strong> 天；生育假 <strong>{fmt(maternityRemain)}</strong> 天；陪产假 <strong>{fmt(paternityRemain)}</strong> 天；其他 <strong>{fmt(otherRemain)}</strong> 天
      </p>
    </div>
  );
};
